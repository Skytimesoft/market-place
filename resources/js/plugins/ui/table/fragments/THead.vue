<template>
    <thead>
        <tr class="bg-gray-100 whitespace-nowrap text-sm" :class="propsData?.theadClass">
            <th 
                v-for="(item, index) in propsData.config"
                :key="index"
                class="group border-b font-medium text-left px-3 py-2 align-middle relative cursor-pointer select-none"
                :class="item.thClass"
                @click="item.sortable && sortData(item.key)"
            >
                {{ item.title }}
                <button v-if="item.sortable" class="absolute right-0 opacity-30 group-hover:opacity-80">
                    <Icon name="PhCaretUpDown" size="16" />
                </button>
            </th>
            <th v-if="hasActionSlot" class="text-right w-10 border-b font-medium px-3 py-2">
                Action
            </th>
        </tr>
    </thead>
</template>

<script setup>
    import { inject } from 'vue'
    import { Icon } from '@/plugins/ui'
    import { sortData } from '../useTable.js'

    defineProps({
        hasActionSlot: {
            type: [Boolean, Function],
            default: false
        }
    })
    const propsData = inject('propsData')
</script>